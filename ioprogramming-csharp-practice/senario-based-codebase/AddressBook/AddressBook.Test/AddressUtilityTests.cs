...
namespace BridgelabzTraining.Tests
{
    ...
    public class AddressUtilityTests
    {
        ...
        [TestMethod]
        public void SortByCity_ShouldSortCorrectly()
        {
            IAddressBook svc = new AddressUtility<UserContact>();
            svc.AddAddressBook("Owner1");

            svc.AddContact("Owner1", ContactFactory.Create("A", "One", "Mumbai", "MH"));
            svc.AddContact("Owner1", ContactFactory.Create("B", "Two", "Delhi", "DL"));
            svc.AddContact("Owner1", ContactFactory.Create("C", "Three", "Pune", "MH"));

            svc.SortContactsByCity("Owner1");

            var contacts = svc.GetContacts("Owner1");
            Assert.AreEqual("Delhi", contacts[0].City());
            Assert.AreEqual("Mumbai", contacts[1].City());
            Assert.AreEqual("Pune", contacts[2].City());
        }

        [TestMethod]
        public void SortByZip_ShouldSortCorrectly()
        {
            IAddressBook svc = new AddressUtility<UserContact>();
            svc.AddAddressBook("Owner1");

            svc.AddContact("Owner1", ContactFactory.Create("A", "One", "City", "State", zip: "500001"));
            svc.AddContact("Owner1", ContactFactory.Create("B", "Two", "City", "State", zip: "400001"));
            svc.AddContact("Owner1", ContactFactory.Create("C", "Three", "City", "State", zip: "600001"));

            svc.SortContactsByZip("Owner1");

            var contacts = svc.GetContacts("Owner1");
            Assert.AreEqual("400001", contacts[0].Zip());
            Assert.AreEqual("500001", contacts[1].Zip());
            Assert.AreEqual("600001", contacts[2].Zip());
        }
