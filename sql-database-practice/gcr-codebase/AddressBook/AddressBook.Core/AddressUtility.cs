...
namespace BridgelabzTraining.senario_based.AddressBook
{
    public class AddressUtility<T> : IAddressBook where T : UserContact
    {
        ...
        /*------------Storing Types--------------------*/
        // Refactoring for UC -13,14,15,16,18 for stroing with json , txt, csv, db
        public void Save(string ownerName, IContactStore store)
        {
            if (store == null) throw new ArgumentNullException(nameof(store));
            var contacts = GetContacts(ownerName);
            store.Save(ownerName, contacts);
        }

        public void Load(string ownerName, IContactStore store)
        {
            if (store == null) throw new ArgumentNullException(nameof(store));
            ...
            var contacts = store.Load(ownerName);
            foreach (var c in contacts)
                book.AddContact((T)c);
        }
