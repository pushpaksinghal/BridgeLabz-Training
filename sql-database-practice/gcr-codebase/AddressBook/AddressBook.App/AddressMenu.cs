...
using BridgeLabzTraining.Core.senario_based.AddressBook.StoringType;
...
namespace BridgelabzTraining.senario_based.AddressBook
{
    internal class AddressMenu
    {
        ...
        private void AddressStarter(string ownerName)
        {
            ...
            while (flag)
            {
                try
                {
                    ...
                    Console.WriteLine("12. Save (choose source: TXT/CSV/JSON/DB)");
                    Console.WriteLine("13. Load (choose source: TXT/CSV/JSON/DB)");
        ...
        private void SaveUI(string ownerName)
        {
            var (type, param) = ReadStoreSelection();
            IContactStore store = ContactStoreFactory.Create(type, param);

            addressBook.Save(ownerName, store);
            Console.WriteLine("Saved successfully.");
        }

        private void LoadUI(string ownerName)
        {
            var (type, param) = ReadStoreSelection();
            IContactStore store = ContactStoreFactory.Create(type, param);

            addressBook.Load(ownerName, store);
            Console.WriteLine("Loaded successfully.");
        }

        private (StoreType type, string param) ReadStoreSelection()
        {
            Console.WriteLine("Choose Storage:");
            Console.WriteLine("1. TXT (pipe text)");
            Console.WriteLine("2. CSV");
            Console.WriteLine("3. JSON File");
            Console.WriteLine("4. SQL Server Database");

            if (!int.TryParse(Console.ReadLine(), out int s))
                throw new FormatException("Invalid storage choice.");

            var type = (StoreType)s;

            if (type == StoreType.SqlServer)
            { 
                return (type, "Server=localhost\\SQLEXPRESS;Database=AddressBook;Trusted_Connection=true;TrustServerCertificate=true;");
            }
